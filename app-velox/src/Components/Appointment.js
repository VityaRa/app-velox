...
function Appointment(props) {
  const doctorsList = props.doctorsList
  const changePressWithCheck = () => {
    if (props.isPressed) {
      props.changePress()
    }
  }
  return (
    <section className="appointment">
      <h3 className="title appointment__title" onClick={changePressWithCheck} style={props.isPressed ? {cursor: "pointer"} : {cursor: "default"}}>
        {!props.isPressed ? null : (
          <svg style={{marginRight: "14px"}}
            width="18"
            height="14"
            viewBox="0 0 18 14"
            fill="none"
